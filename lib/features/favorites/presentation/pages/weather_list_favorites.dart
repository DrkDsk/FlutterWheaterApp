...
class WeatherListFavorites extends StatelessWidget {
  ...
  void _onLoadWeather(BuildContext context, CityWeatherState state, Color? backgroundColor) {
    if (state is FetchWeatherLoadingState) {
      ...
      _showWeatherBottomSheet(context, cityName: state.cityName, latitude: state.latitude, longitude: state.longitude, backgroundColor: backgroundColor);
    }
  }
    ...
    return BlocSelector<CityWeatherBloc, CityWeatherState, Color?>(
      ...
      builder: (context, backgroundColor) {
        return BlocListener<CityWeatherBloc, CityWeatherState>(
          listener: (context, state) => _onLoadWeather(context, state, backgroundColor),
